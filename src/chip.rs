...
//             struct $circuit_name<F: BigPrimeField> {
//                 n: BigUint,
//                 e: BigUint,
//                 x: BigUint,
//                 _f: PhantomData<F>
...
//             impl<F: BigPrimeField> $circuit_name<F> {
...
//             impl<F: BigPrimeField> Circuit<F> for $circuit_name<F> {
...
//                     let range_config = RangeChip::configure(meta,Vertical, &[Self::NUM_ADVICE], &[Self::NUM_LOOKUP_ADVICE], Self::NUM_FIXED, Self::LOOKUP_BITS, 0, $k);
...
//                     let ctx = &mut aux;
//                     let e_var = RSAPubE::Var(self.e.clone());
//                     let e_fix = RSAPubE::Fix(BigUint::from_u128(Self::DEFAULT_E).unwrap());
//                     let public_key_var = RSAPublicKey::new(self.n.clone(), e_var);
//                     let public_key_var = config.assign_public_key(ctx, public_key_var)?;
//                     let public_key_fix = RSAPublicKey::new(self.n.clone(), e_fix);
